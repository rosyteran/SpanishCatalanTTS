...
from typing import Any, Callable, List, Optional, Tuple, Type, TypeVar, Union, Dict
...
def stft(x, fft_size, hop_size, win_length, window, device):
...
class SpecDiscriminator(nn.Module):
    ...
    def forward(self, y):
        ...
        fmap = []
        y = y.squeeze(1)
        y = stft(y, self.fft_size, self.shift_size, self.win_length, self.window.to(y.get_device() if y.is_cuda else "cpu"), y.get_device() if y.is_cuda else "cpu")
        y = y.unsqueeze(1)
...
class StyleTransformer1d(nn.Module):
    ...
    def get_mapping(
        ...
        if self.use_context_time:
            assert_message = "use_context_time=True but no time features provided"
            assert exists(time), assert_message
            time_features = self.to_time(time)
            
            # Ensure time_features has the right shape
            if time_features.dim() == 1:
                time_features = time_features.unsqueeze(0)
            
            items.append(time_features)
        
        # Compute features
        if self.use_context_features:
            assert_message = "context_features exists but no features provided"
            assert exists(features), assert_message
            feature_embedding = self.to_features(features)
            
            # Ensure feature_embedding has the right shape
            if feature_embedding.dim() == 1:
                feature_embedding = feature_embedding.unsqueeze(0)
            
            items.append(feature_embedding)
...
class ConvNorm(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=1, stride=1,
        ...
        nn.init.xavier_uniform_(
            self.conv.weight, gain=nn.init.calculate_gain(w_init_gain, param=param))
...
class Generator(torch.nn.Module):
    def __init__(self, style_dim, resblock_kernel_sizes, upsample_rates, upsample_initial_channel, resblock_dilation_sizes, upsample_kernel_sizes):
        ...
        self.m_source = SourceModuleHnNSF(
                    sampling_rate=24000,
                    upsample_scale=float(np.prod(upsample_rates)),
...
class Decoder(nn.Module):
    ...
    def forward(self, asr, F0_curve, N, s, device='cuda'):
        if self.training:
            downlist = [0, 3, 7]
...
class DecoderConfig:
    def __init__(self, resblock_dilation_sizes=None, resblock_kernel_sizes=None, upsample_kernel_sizes=None, upsample_rates=None, type="hifigan", upsample_initial_channel=512):
        self.type = type
...
class SLMConfig:
    def __init__(self, hidden=768, initial_channel=64, model="microsoft/wavlm-base-plus", nlayers=12, sr=16000):
        self.hidden = hidden
...
class Dist:
    def __init__(self, estimate_sigma_data, mean, sigma_data, std):
        self.estimate_sigma_data = estimate_sigma_data
        self.mean = mean
        self.sigma_data = sigma_data
        self.std = std
...
class DiffusionConfig:
    def __init__(self, dist, transformer, embedding_mask_proba=0.1):
        self.dist = Dist(**dist)
        self.embedding_mask_proba = embedding_mask_proba
        self.transformer = transformer

class StyleTTS2CatalanConfig:
    def __init__(self, asr_config, bert, slm, **kwargs):
        self.asr_config = asr_config
        self.bert = AlbertConfig(**bert)
        self.decoder = DecoderConfig(**kwargs.get('decoder', {}))
        self.diffusion = DiffusionConfig(**kwargs.get('diffusion', {}))
        self.slm = SLMConfig(**slm)
        self.dim_in = kwargs.get('dim_in', 64)
        self.dropout = kwargs.get('dropout', 0.2)
        self.hidden_dim = kwargs.get('hidden_dim', 512)
        self.max_conv_dim = kwargs.get('max_conv_dim', 512)
        self.max_dur = kwargs.get('max_dur', 50)
        self.multispeaker = kwargs.get('multispeaker', True)
        self.n_layer = kwargs.get('n_layer', 3)
        self.n_mels = kwargs.get('n_mels', 80)
        self.n_token = kwargs.get('n_token', 178)
        self.style_dim = kwargs.get('style_dim', 128)

class StyleTTS2Encoder(nn.Module):
    def __init__(self, args):
        super().__init__()
        self.style_encoder = StyleEncoder(dim_in=args.dim_in, style_dim=args.style_dim, max_conv_dim=args.hidden_dim)
        self.predictor_encoder = StyleEncoder(dim_in=args.dim_in, style_dim=args.style_dim, max_conv_dim=args.hidden_dim)
    ...
    def forward(self, wave):
        """
        Wave is 24KHz Resampled audio tensor or np (Unsure)
        """
        mel_tensor = self.to_mel(wave)
        mel_tensor = (torch.log(1e-5 + mel_tensor.unsqueeze(0)) - self.mean) / self.std
        ref_style = self.style_encoder(mel_tensor.unsqueeze(1))
        ref_predictor = self.predictor_encoder(mel_tensor.unsqueeze(1))
        return torch.cat([ref_style, ref_predictor], dim=1)

class StyleTTS2Catalan(nn.Module):
    def __init__(self, args):
        super().__init__()
        self.bert = CustomAlbert(args.bert)
        self.bert_encoder=nn.Linear(args.bert.hidden_size, args.hidden_dim)
        ...
        # self.style_encoder = StyleEncoder(dim_in=args.dim_in, style_dim=args.style_dim, max_conv_dim=args.hidden_dim)
        # self.predictor_encoder = StyleEncoder(dim_in=args.dim_in, style_dim=args.style_dim, max_conv_dim=args.hidden_dim)
        self.transformer = StyleTransformer1d(channels=args.style_dim*2, 
                                    context_embedding_features=args.bert.hidden_size,
                                    context_features=args.style_dim*2, 
                                    **args.diffusion.transformer)
        self.diffusion = AudioDiffusionConditional(
            in_channels=1,
            embedding_max_length=args.bert.max_position_embeddings,
            embedding_features=args.bert.hidden_size,
        ...
        self.diffusion.diffusion.net = self.transformer
        self.diffusion.unet = self.transformer
        self.mpd = MultiPeriodDiscriminator()
        self.msd = MultiResSpecDiscriminator()
        self.wd = WavLMDiscriminator(args.slm.hidden, args.slm.nlayers, args.slm.initial_channel)
        ...
        self.text_aligner = ASRCNN(**args.asr_config)
        self.pitch_extractor = JDCNet(num_class=1, seq_len=192)

    def forward(self, tokens, input_lengths, ref_s, alpha=0.3, beta=0.7, 
                diffusion_steps=5, embedding_scale=1, device='cuda', **kwargs):
        # Prepare text mask
        ...
        # Duration prediction with BERT and subsequent processing
        bert_dur = self.bert(tokens, attention_mask=(~text_mask).int())
        d_en = self.bert_encoder(bert_dur).transpose(-1, -2)
        ref_s = ref_s.to(device)
        # Sample from the latent space using diffusion
        s_pred = self.sampler(
        ...
        # Split latent variables into `s` and `ref`
        s = s_pred[:, 128:]
        ref = s_pred[:, :128]

        # Interpolate with reference speaker embedding
        ref = alpha * ref + (1 - alpha) * ref_s[:, :128]
        s = beta * s + (1 - beta) * ref_s[:, 128:]

        # Duration and alignment predictions
        ...
        # Decoder output
        out = self.decoder(asr, F0_pred, N_pred, ref.squeeze().unsqueeze(0), device=device)
